...
const rodizioOverview = () => {
  const principal=Object.values(rodizioGroups.principal)
    .map((g)=>`${g.titulo}: ${g.resumo}`).join('\n\n');
  const guia=Object.keys(rodizioPreferencias).map(preferenceText).join('\n\n');
  return `Nosso cadastro de rodízio tem muitas opções! 🍣\n\n${principal}\n\n`+
    `— GUIA POR PREFERÊNCIA (separado da lista principal) —\n${guia}\n\n`+
    `Esses grupos organizam itens do próprio rodízio, não são pratos extras. `+
    `Bebidas e sobremesas são cobradas à parte. `+
    `Os Joys Especiais sem arroz do cardápio à la carte não estão confirmados no rodízio. `+
    `Confirme a disponibilidade de cada item com nossa equipe.`;
};
...
export function resolve(input={}) {
  ...
  const n = norm(raw.replace(/rodiozio|rodizzio|rodizo|rrodizio/g,'rodizio'));
  ...
  } else if (/\brodizio\b/.test(n) && rodizioDetailQuestion(n) ||
    s.last_topic==='rodizio' && rodizioDetailQuestion(n)) {
    r=result('rodizio_composicao',`${rodizioOverview()}\n\n${rodizioPriceSummary()}`,
      [MENU,WA],{topic:'rodizio'});
  } else if (/\b(rodizio)\b/.test(n)) {
    r=result('rodizio',
      `Temos rodízio todos os dias! Individual: ${knowledge.rodizio.individual}. Casal: ${knowledge.rodizio.casal}. Infantil (9 a 11 anos): ${knowledge.rodizio.infantil_9_a_11}; menores de 9 anos não pagam. Bebidas e sobremesas à parte. Se quiser, posso te mostrar também os fritos, grelhados e opções sem arroz.`,
  ...
  } else if (/\b(?:i ?food|99 ?food)\b/.test(n)) {
    const platform=/\b99 ?food\b/.test(n)?'food99':'ifood';
  ...
  } else if (/\b(entrega|delivery|retirada|retirar|buscar|taxa|frete|bairro|motoboy)\b/.test(n)) {
  ...
  try {
    const http=await fetchImpl('https://api.openai.com/v1/responses',{
      ...
      body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-4o-mini',store:false,
        instructions:persona,input:JSON.stringify({intent:r.intent,resposta_autorizada:r.reply}),
        text:{format:{type:'json_object'}},max_output_tokens:160})
    });
    if (!http.ok) {console.warn('OPENAI_REPLY_ERROR',http.status);return r;}
...
export function standardPayload(r) {
  ...
  const payload={ok:true,app_version:VERSION,channel:r.channel,reply,
    ...
    cta_count:cs.length,ctas:cs,ai_state:JSON.stringify(r.state),
